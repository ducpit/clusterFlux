...
echo Acquire::http::Proxy \"http://$nutzername:$passwort@proxy.h-ka.de:8888/\"\; > /etc/apt/apt.conf.d/apt.conf
echo Acquire::https::Proxy \"http://$nutzername:$passwort@proxy.h-ka.de:8888/\"\; >> /etc/apt/apt.conf.d/apt.conf
